...
function buildPasswordPreservationKey(values: {
  ...
  return JSON.stringify({
    ...
    remoteRoot: values.remoteRoot ?? "cc-gateway-pro-sync",
...
export function WebdavSyncSection({
  ...
  const [form, setForm] = useState(() => ({
    ...
    remoteRoot: config?.remoteRoot ?? "cc-gateway-pro-sync",
  ...
  useEffect(() => {
    ...
    setForm(() => {
      ...
      const nextRemoteRoot = config.remoteRoot ?? "cc-gateway-pro-sync";
  ...
  const buildSettings = useCallback((): WebDavSyncSettings | null => {
    ...
    return {
      ...
      remoteRoot: form.remoteRoot.trim() || "cc-gateway-pro-sync",
  ...
  const currentRemotePath = `/${form.remoteRoot.trim() || "cc-gateway-pro-sync"}/v2/db-v6/${form.profile.trim() || "default"}`;
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-4">
            ...
            <Input
              ...
              placeholder="cc-gateway-pro-sync"
              className="text-xs flex-1"
              disabled={isLoading}
            />
          </div>
