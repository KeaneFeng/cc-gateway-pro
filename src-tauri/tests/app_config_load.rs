...
use cc_gateway_pro_lib::{AppError, MultiAppConfig};
...
fn cfg_path() -> PathBuf {
    ...
    PathBuf::from(home).join(".cc-gateway-pro").join("config.json")
}
...
fn load_v1_config_returns_error_and_does_not_write() {
    ...
    let bak = home.join(".cc-gateway-pro").join("config.json.bak");
...
fn load_valid_v2_config_succeeds() {
    ...
    assert!(loaded
        .get_manager(&cc_gateway_pro_lib::AppType::Claude)
        .is_some());
    assert!(loaded.get_manager(&cc_gateway_pro_lib::AppType::Codex).is_some());
}
