...
fn settings_path(home: &Path) -> PathBuf {
    home.join(".cc-gateway-pro").join("settings.json")
}
...
fn switch_provider_updates_claude_live_and_state() {
    ...
    let settings_path = cc_gateway_pro_lib::get_claude_settings_path();
    ...
    let db_path = std::path::Path::new(&home_dir)
        .join(".cc-gateway-pro")
        .join("cc-gateway-pro.db");
    assert!(
        db_path.exists(),
        "switching provider should persist to cc-gateway-pro.db"
    );
