...
#[allow(dead_code)]
pub const CC_GATEWAY_PRO_CODEX_MODEL_PROVIDER_ID: &str = "custom";
pub const CC_GATEWAY_PRO_CODEX_MODEL_CATALOG_FILENAME: &str = "cc-switch-model-catalog.json";
...
pub fn get_codex_model_catalog_path() -> PathBuf {
    get_codex_config_dir().join(CC_GATEWAY_PRO_CODEX_MODEL_CATALOG_FILENAME)
}
...
pub fn prepare_codex_config_text_with_model_catalog(
    ...
    if let Some(catalog) = codex_model_catalog_from_settings(settings, config_text)? {
        ...
        let is_native_responses =
            settings.get("apiFormat").and_then(|value| value.as_str()) == Some("openai_responses");
        let disable_web_search =
            is_native_responses && codex_native_gateway_rejects_web_search(&config_text);
