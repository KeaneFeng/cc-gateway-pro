...
///
/// 原因：接管模式下 `*_MODEL` 必须由 CC Gateway Pro 写成稳定的 Claude 角色别名，
...
impl ProxyService {
    ...
    /// 统一判断指定应用的 Live 配置是否已被代理占位符接管。
    /// 用于 backup_live_configs / backup_live_config_strict 中跳过已被代理接管的 Live，
    /// 避免把代理占位符当作"原始 Live"存进备份槽。
...
mod tests {
    ...
    fn managed_account_claude_takeover_sources_codex_models_from_provider() {
        ...
        assert_env_str(env, "ANTHROPIC_API_KEY", Some(PROXY_TOKEN_PLACEHOLDER));
        // Codex（非 Copilot）必须无条件注入 AUTH_TOKEN 占位符（upstream: 2d64d8c6）
