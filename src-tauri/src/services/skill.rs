...
//!
//! v3.10.0+ 统一管理架构：
//! - SSOT（单一事实源）：`~/.cc-gateway-pro/skills/`
...
impl SkillService {
    ...
    /// 获取 Skill 卸载备份目录（~/.cc-gateway-pro/skill-backups/）
    ...
    pub fn scan_unmanaged(db: &Arc<Database>) -> Result<Vec<UnmanagedSkill>> {
        ...
        if let Ok(ssot_dir) = Self::get_ssot_dir() {
            scan_sources.push((ssot_dir, "cc-gateway-pro".to_string()));
        }
    ...
    pub fn import_from_apps(
        ...
        search_sources.push((ssot_dir.clone(), "cc-gateway-pro".to_string()));
