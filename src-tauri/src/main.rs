...
fn main() {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        // 设置 CC_GATEWAY_PRO_GDK_BACKEND=wayland 即可强制覆盖；同时兼容上游
        // CC_SWITCH_GDK_BACKEND，默认行为保持不变（零回归）。
        if let Ok(backend) = std::env::var("CC_GATEWAY_PRO_GDK_BACKEND")
            .or_else(|_| std::env::var("CC_SWITCH_GDK_BACKEND"))
        {
            if !backend.is_empty() {
