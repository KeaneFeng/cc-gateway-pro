...
fn handle_deeplink_url(
    ...
    if !url_str.starts_with("ccgatewaypro://") {
        return false;
    }
...
pub fn run() {
    // 设置 panic hook，在应用崩溃时记录日志到 <app_config_dir>/crash.log（默认 ~/.cc-gateway-pro/crash.log）
    ...
    let builder = builder
        ...
        .setup(|app| {
            ...
            let db_path = app_config_dir.join("cc-gateway-pro.db");
            ...
            #[cfg(any(target_os = "linux", all(debug_assertions, windows)))]
            {
                #[cfg(target_os = "linux")]
                {
                    ...
                    // tauri-plugin-deep-link writes to: ~/.local/share/com.ccgatewaypro.desktop/applications/cc-gateway-pro-handler.desktop
                    // Only register if .desktop file doesn't exist to avoid overwriting user customizations
                    let should_register = app
                        .path()
                        .data_dir()
                        .map(|d| !d.join("applications/cc-gateway-pro-handler.desktop").exists())
                        .unwrap_or(true);
                ...
                move |event| {
                    ...
                    for (i, url) in urls.iter().enumerate() {
                        ...
                        if handle_deeplink_url(&app_handle, url_str, true, "on_open_url") {
                            break; // Process only first ccgatewaypro:// URL
                        }
                    }
                }
            });
    ...
    app.run(|app_handle, event| {
        ...
        #[cfg(target_os = "macos")]
        {
            match event {
                ...
                // 处理通过自定义 URL 协议触发的打开事件（例如 ccgatewaypro://...）
...
fn show_database_init_error_dialog(
    ...
    let message = if is_chinese_locale() {
        format!(
            ...
            建议：\n\
            1) 先备份整个配置目录（包含 cc-gateway-pro.db）\n\
