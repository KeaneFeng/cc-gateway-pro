...
mod tests {
    ...
    fn test_get_macos_app_bundle_path_valid() {
        let exe_path = std::path::Path::new("/Applications/CC-Gateway-Pro.app/Contents/MacOS/CC-Gateway-Pro");
    ...
    fn test_get_macos_app_bundle_path_with_spaces() {
        let exe_path =
            std::path::Path::new("/Users/test/My Apps/CC-Gateway-Pro.app/Contents/MacOS/CC-Gateway-Pro");
        let result = get_macos_app_bundle_path(exe_path);
        assert_eq!(
            result,
    ...
    fn test_get_macos_app_bundle_path_not_in_bundle() {
        let exe_path = std::path::Path::new("/usr/local/bin/cc-gateway-pro");
        let result = get_macos_app_bundle_path(exe_path);
    ...
    fn test_get_macos_app_bundle_path_dev_build() {
        ...
        let exe_path = std::path::Path::new("/Users/dev/project/target/debug/cc-gateway-pro");
        let result = get_macos_app_bundle_path(exe_path);
        assert_eq!(result, None);
    }
}
