...
//!
//! This module is used when the Codex client talks to CC Gateway Pro through the
        ...
        match part_type {
            ...
            "input_image" => {
                if let Some(image_url) = part.get("image_url") {
                    let url_str = if image_url.is_object() {
                        image_url.get("url").and_then(|u| u.as_str()).unwrap_or("")
                    } else {
                        image_url.as_str().unwrap_or("")
                    };
                    log::info!(
                        "[Transform] input_image detected, url_len={}, has_data_prefix={}",
                        url_str.len(),
                    ...
                    log::warn!(
                        "[Transform] input_image part has NO image_url field: keys={:?}",
