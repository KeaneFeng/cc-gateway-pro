...
fn codex_proxy_error_json(
    ...
    let message = if upstream_status == Some(413) {
        ...
        // Gateway Pro 本地代理的限制（本地 DefaultBodyLimit 已放到 200MB）。上游响应体往往是
        // 一整段 nginx HTML，对用户毫无价值，这里替换成明确指向上游 + 可操作的指引，
        // 避免「以为是 CC Gateway Pro 封装了 nginx / 是本地代理的锅」这种反复出现的误解。
        format!(
            concat!(
                ...
                "provider's server-side limit, not a CC Gateway Pro limit. ",
        ...
        let status_fragment = upstream_status
            ...
            .unwrap_or_default();
        format!(
            "CC Gateway Pro local proxy failed while handling Codex endpoint {endpoint}. Provider: {provider_name}; model: {request_model}{status_fragment}; cause: {cause}"
        )
    };
...
fn codex_proxy_error_code(error: &ProxyError) -> &'static str {
    match error {
        ProxyError::ForwardFailed(_) => "cc_gateway_pro_forward_failed",
        ProxyError::Timeout(_) | ProxyError::StreamIdleTimeout(_) => "cc_gateway_pro_timeout",
        ProxyError::NoAvailableProvider => "cc_gateway_pro_no_available_provider",
        ProxyError::AllProvidersCircuitOpen => "cc_gateway_pro_all_providers_circuit_open",
        ProxyError::NoProvidersConfigured => "cc_gateway_pro_no_providers_configured",
        ProxyError::MaxRetriesExceeded => "cc_gateway_pro_max_retries_exceeded",
        ProxyError::ProviderUnhealthy(_) => "cc_gateway_pro_provider_unhealthy",
        ProxyError::ConfigError(_) => "cc_gateway_pro_config_error",
        ProxyError::TransformError(_) => "cc_gateway_pro_transform_error",
        ProxyError::InvalidRequest(_) => "cc_gateway_pro_invalid_request",
        ProxyError::AuthError(_) => "cc_gateway_pro_auth_error",
        ProxyError::UpstreamError { .. } => "cc_gateway_pro_upstream_error",
        ProxyError::DatabaseError(_) => "cc_gateway_pro_database_error",
        ProxyError::Internal(_) => "cc_gateway_pro_internal_error",
        ProxyError::AlreadyRunning
        | ProxyError::NotRunning
        | ProxyError::BindFailed(_)
        | ProxyError::StopTimeout
        | ProxyError::StopFailed(_) => "cc_gateway_pro_proxy_error",
    }
}
...
data: {\"type\":\"response.output_item.done\",\"item\":{\"type\":\"message\"}}\n\n";
    ...
    fn codex_proxy_forward_error_includes_context_and_cause() {
        ...
        assert!(message.contains("CC Gateway Pro local proxy failed"));
        ...
        assert_eq!(body["error"]["code"], "cc_gateway_pro_forward_failed");
