...
fn proxy_points_to_loopback(value: &str) -> bool {
    ...
    // 检查是否指向 CC-Gateway-Pro 自己的代理端口
    // 只有指向自己的代理才需要跳过，避免递归
    fn is_cc_gateway_pro_proxy_port(port: Option<u16>) -> bool {
        let cc_gateway_pro_port = get_proxy_port();
        port == Some(cc_gateway_pro_port)
    }

    if let Ok(parsed) = url::Url::parse(value) {
        if let Some(host) = parsed.host_str() {
            // 只有当主机是 loopback 且端口是 CC-Gateway-Pro 的端口时才返回 true
            return host_is_loopback(host) && is_cc_gateway_pro_proxy_port(parsed.port());
        }
        return false;
    }
...
mod tests {
    ...
    fn test_proxy_points_to_loopback() {
        // 设置 CC-Gateway-Pro 代理端口为 15721（默认值）
        set_proxy_port(15721);

        // 只有指向 CC-Gateway-Pro 自己端口的 loopback 地址才返回 true
