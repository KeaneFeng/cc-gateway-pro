...
## CC-Gateway-Pro のデータストレージ

### ストレージディレクトリ

デフォルトの場所：`~/.cc-gateway-pro/`
...
```
~/.cc-gateway-pro/
├── cc-gateway-pro.db      # SQLite データベース（SSOT）
...
`cc-gateway-pro.db` は SQLite データベースで、以下を保存しています：
...
| フィールド | 説明 |
|------|------|
| `models.providers` | プロバイダー設定（CC-Gateway-Pro の「プロバイダー」にマッピング） |
| `env` | 環境変数設定 |
...
CC-Gateway-Pro が設定を変更する際の優先順位：

1. **CC-Gateway-Pro データベース** - 単一事実源 (SSOT)
...
- CLI ツールの設定ファイル（CC-Gateway-Pro がバックフィルする）
- CC-Gateway-Pro の `settings.json`

### 手動編集を推奨しないもの

- `cc-gateway-pro.db` データベースファイル
- バックアップファイル
...
CC-Gateway-Pro v3.7.0 で JSON ファイルから SQLite に移行しました：
