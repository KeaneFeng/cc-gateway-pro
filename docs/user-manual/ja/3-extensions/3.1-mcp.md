...
あるアプリのスイッチをオンにすると、CC-Gateway-Pro は以下を実行します：
...
あるアプリのスイッチをオフにすると、CC-Gateway-Pro は以下を実行します：
...
CLI ツールで既に MCP サーバーを設定している場合、CC-Gateway-Pro にインポートできます：

1. 「インポート」ボタンをクリック
2. インポートするアプリを選択（Claude/Codex/Gemini/OpenCode）
3. CC-Gateway-Pro が既存の設定を読み取ってインポート
