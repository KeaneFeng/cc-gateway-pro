...
This section describes how to configure CC-Gateway-Pro according to your preferences.
...
CC-Gateway-Pro supports three languages:
...
When enabled, CC-Gateway-Pro automatically runs when the system starts.
...
Starting from v3.13.0, CC-Gateway-Pro adds **Lightweight Mode** — a **tray-only** running state that minimizes desktop footprint when idle.
...
**How to exit**: Click **Open Main Window** from the tray menu, or trigger CC-Gateway-Pro via deep link / relaunch. The window is **rebuilt on demand**, with state preserved.
...
> **Use case**: If CC-Gateway-Pro runs in the background for long periods and you mainly switch providers via the tray menu, enabling Lightweight Mode significantly reduces memory usage.
...
When enabled, CC-Gateway-Pro automatically syncs the configuration to the VS Code Claude Code extension (writes `primaryApiKey` to `~/.claude/config.json`) when switching providers.
...
Choose the terminal application that CC-Gateway-Pro uses when opening a terminal.
...
The storage location for CC-Gateway-Pro's own data, defaulting to `~/.cc-gateway-pro/`.
...
Configure CC-Gateway-Pro's outbound HTTP/HTTPS proxy, applicable for scenarios where external API access requires a proxy.
...
When an interval is set, CC-Gateway-Pro automatically backs up the database on schedule. Older backups beyond the retention count are automatically removed.
...
| Remote Directory | Remote storage path (default: `cc-gateway-pro-sync`) |
...
- CC-Gateway-Pro automatically syncs the database to WebDAV at the configured interval
- Sync status is displayed in the panel

#### Remote Snapshot Info

Before downloading, CC-Gateway-Pro displays details about the remote snapshot:
- Protocol version (v2)
...
Click **View Logs** to open the proxy request log viewer in your preferred terminal. Use it when debugging local proxy routing, status codes, model selection, provider selection, latency, or errors. Structured usage and cost analysis remains available in the Usage panel.

### Session Traces

Session Traces lives in Advanced settings and controls session context analysis capture. It is off by default and records only new requests after you enable it.

| Setting | Description |
|---------|-------------|
| Enable Session Traces | Records new context summaries, tool calls, and per-turn usage |
| Capture mode: Summary | Stores extracted fields, context categories, tool/Skills/MCP stats, and response previews |
| Capture mode: Full | Stores redacted request/response JSON for deeper debugging on trusted devices |
| Default retention | 14 days |
| Response preview | 2000 characters |

> **Note**: Session Traces data stays local. Sensitive keys are redacted, but Full mode stores more request and response detail, so enable it only on trusted devices.
...
Displays the current CC-Gateway-Pro version number, with support for:
