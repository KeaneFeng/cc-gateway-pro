...
App routing means letting CC-Gateway-Pro route a specific application's API requests through the local routing service.
...
When routing is enabled, CC-Gateway-Pro modifies the app's configuration file to point the API endpoint to the local routing service.
...
When disabling routing, CC-Gateway-Pro will:
...
Before enabling routing, CC-Gateway-Pro backs up the original configuration and restores it when disabled.
