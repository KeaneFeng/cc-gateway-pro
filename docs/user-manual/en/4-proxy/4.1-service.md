...
sequenceDiagram
    ...
    participant Proxy as Local Proxy (CC-Gateway-Pro)
...
After starting the proxy and enabling app takeover, CC-Gateway-Pro modifies app configurations:
...
When stopping the proxy, CC-Gateway-Pro will:
...
Click **View Logs** in **Settings > Advanced > Log Configuration** to open the last 7 days of proxy request logs in your preferred terminal with real-time scrolling.

The Usage panel still provides structured request records, token usage, costs, and trends. View Logs is better for watching live request flow, status codes, model routing, provider selection, and errors.
