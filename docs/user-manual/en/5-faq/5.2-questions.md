...
If you see a "damaged app" or "unverified developer" warning when installing from the DMG, this is macOS Gatekeeper blocking an unsigned app. Run the following command to remove the quarantine attribute:

```bash
xattr -cr /Applications/CC\ Gateway\ Pro.app
```

Then reopen the app — it will launch normally.

> **Recommended**: Install via Homebrew to avoid this issue entirely:
> ```bash
...
2. Add CC-Gateway-Pro to your antivirus software's whitelist
...
**Solutions**:
1. Check if the `~/.cc-gateway-pro/` directory exists
2. Restore from backup: `~/.cc-gateway-pro/backups/`
...
**Solutions**:
1. Confirm the file was exported by CC-Gateway-Pro
...
**Windows**:
- Check taskbar settings to ensure the CC-Gateway-Pro icon is not hidden
...
2. Restart the app
3. Delete `~/.cc-gateway-pro/settings.json` to reset settings
...
3. If using Homebrew: `brew upgrade --cask cc-gateway-pro`

## Lightweight Mode

### How to Enter Lightweight Mode?

Toggle "Lightweight Mode" from the system tray menu. The main window closes, and CC-Gateway-Pro runs as a tray-only app. Toggle again or click "Open main window" to exit.
...
Yes. Starting from v3.13.0, CC-Gateway-Pro covers all window re-show paths (normal launch, deep links, singleton activation, tray `show_main`, and Lightweight Mode return). Clicking a `ccgatewaypro://` link **rebuilds the main window on demand** and displays the import confirmation dialog. The first open is slightly slower than normal state (window rebuild required), but subsequent switches return to normal speed.
...
1. Visit [GitHub Issues](https://github.com/KeaneFeng/cc-gateway-pro/issues)
2. Search for similar issues
3. If none found, create a new Issue
4. Provide the following information:
   - Operating system and version
   - CC-Gateway-Pro version
...
- macOS/Linux: `~/.cc-gateway-pro.logs/`
- Windows: `%APPDATA%\cc-gateway-pro.logs\`
