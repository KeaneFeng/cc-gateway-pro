...
## CC-Gateway-Pro Data Storage

### Storage Directory

Default location: `~/.cc-gateway-pro/`
...
```
~/.cc-gateway-pro/
├── cc-gateway-pro.db      # SQLite database (SSOT)
...
`cc-gateway-pro.db` is a SQLite database that stores:
...
| Field | Description |
|-------|-------------|
| `models.providers` | Provider configuration (mapped to CC-Gateway-Pro's "providers") |
...
CC-Gateway-Pro's priority when modifying configurations:

1. **CC-Gateway-Pro Database** - Single source of truth (SSOT)
...
- CLI tool configuration files (will be backfilled by CC-Gateway-Pro)
- CC-Gateway-Pro's `settings.json`

### Not Recommended to Edit Manually

- `cc-gateway-pro.db` database file
- Backup files
...
CC-Gateway-Pro v3.7.0 migrated from JSON files to SQLite:
