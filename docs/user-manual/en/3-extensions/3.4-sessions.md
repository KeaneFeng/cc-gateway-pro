...
The Session Traces entry sits next to Session Manager in the main navigation. Use it to analyze context quality, tool/Skills/MCP usage, and per-turn usage. It is separate from Session Manager: Session Manager browses, resumes, and deletes local sessions, while Session Traces analyzes new requests captured after you enable tracing.

Session Traces is off by default. After you enable it, new requests are recorded in the local database. When it is off, existing historical usage remains visible, but new context traces are not recorded.

| Mode | Description |
|------|-------------|
| Summary | Stores extracted fields, context categories, tool calls, and response previews |
| Full | Stores redacted request/response JSON for deeper debugging on trusted devices |

Default retention is 14 days, and response previews default to 2000 characters. Sensitive keys are redacted, but Full mode should only be enabled on trusted devices.
...
- **Session Traces**: For linkable historical sessions, the analysis action opens usage and context stats for that session
...
**On macOS:**
- CC-Gateway-Pro launches your preferred terminal with the resume command
...
3. CC-Gateway-Pro launches the Claude terminal session in the selected directory
