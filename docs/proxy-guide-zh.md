...
CC-Gateway-Pro 的代理功能是在本机启动一个 HTTP 网关，默认监听 `127.0.0.1:15721`。启用应用接管后，Claude Code、Claude Desktop、Codex 或 Gemini CLI 的请求会先进入本地代理，再由代理选择实际 Provider、转换 API 格式、记录用量与请求日志，并在需要时执行项目级路由、Vision Model 自动路由、Session Traces 和故障转移。
...
- 希望在终端中实时查看最近 7 天的代理请求日志。
- 希望按需采集会话上下文摘要、工具调用和每轮 usage。
- 希望使用自动故障转移和熔断器提高可用性。
...
在「设置 → 高级 → 日志配置」中点击 **View Logs**，CC-Gateway-Pro 会使用你偏好的终端打开代理请求日志查看器，实时滚动显示最近 7 天的代理请求日志。日志文件会按保留策略自动清理，适合排查请求路径、状态码、模型、Provider、延迟和错误。

用量仪表盘仍然会展示结构化请求记录、Token、成本和趋势；View Logs 更适合持续观察正在发生的代理请求。

### Session Traces

Session Traces 是比普通请求日志更深入的上下文分析视图。它默认关闭，只有开启后才会记录新的上下文摘要、工具调用和每轮 usage。

| 配置项 | 说明 |
| ------ | ---- |
| Enable Session Traces | 开启后开始记录新的 Session Traces；关闭时只保留已有历史统计 |
| Capture mode: Summary | 仅保存提取字段、上下文分类、工具/Skills/MCP 统计和响应预览 |
| Capture mode: Full | 保存脱敏后的 request/response JSON，适合可信设备上的深度排查 |
| 默认保留 | 14 天 |
| 响应预览 | 2000 字符 |

> Session Traces 数据只保存在本机，敏感 key 会脱敏。由于 Full 模式会保存更多请求/响应细节，请只在可信设备上启用。
